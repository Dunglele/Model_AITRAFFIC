...
def process_video(source_path, output_path, model_id="annonate_datatftphcm/1"):
    ...
    def callback(frame: np.ndarray, index: int) -> np.ndarray:
        ...
        cv2.putText(annotated_frame, f"OC-SORT Density: {occ_score:.1f}% ({level})", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (255, 0, 255), 2)
    ...
    with sv.VideoSink(target_path=output_path, video_info=new_video_info, codec="MJPG") as sink:
        for index, frame in enumerate(tqdm(generator, total=video_info.total_frames, desc="OC-SORT Analysis")):
    ...
    if not df.empty:
        ...
        plt.plot(df['time'], df['density'], color='purple', linewidth=2, marker='^', label='OC-SORT Density (%)')
