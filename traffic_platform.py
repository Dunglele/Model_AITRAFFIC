...
def process_video(
    source_path, output_path, mode="fast", model_id="annonate_datatftphcm/1"
):
    ...
    if mode == "mask":
        ...
        if mode == "fast":
            ...
            tracker = DeepSort(
                max_age=15, nn_budget=50, embedder="mobilenet", embedder_gpu=False
            )
    ...
    def callback(frame, index):
        ...
        if index % stride == 0:
            ...
            if mode == "stable":
                ...
                if len(m_idx) > 0:
                    det = sv.Detections(
                        xyxy=outputs["boxes"][m_idx].cpu().numpy(),
                        class_id=outputs["labels"][m_idx].cpu().numpy(),
                        confidence=outputs["scores"][m_idx].cpu().numpy(),
                        mask=(outputs["masks"][m_idx] > 0.5).cpu().numpy().squeeze(1),
                    )
        ...
        if not det_in.is_empty():
            if mode == "mask" and det_in.mask is not None:
                ...
                for tid in det_in.tracker_id:
                    unique_ids.add(tid)

        occ = min(
            (
                (curr_pce * 15000 / road_area) * 100
                if mode != "mask"
                else (curr_pce * 100 / road_area) * 100
            ),
            100.0,
        )
        level = get_traffic_level(occ)
        if index % int(video_info.fps) == 0:
            analytics_data.append(
                {"time": index // int(video_info.fps), "density": occ}
            )

        # Render
        annotated = fr.copy()
        if mode == "mask" and not last_det.is_empty():
            annotated = mask_annotator.annotate(scene=annotated, detections=last_det)
        else:
            annotated = box_annotator.annotate(scene=annotated, detections=last_det)

        annotated = zone_annotator.annotate(scene=annotated)
        cv2.putText(
            annotated,
            f"{mode.upper()} | D: {occ:.1f}% ({level})",
            (10, 20),
    ...
    with sv.VideoSink(
        ...
        for index, frame in enumerate(
            tqdm(generator, total=video_info.total_frames, desc=f"Running {mode}")
        ):
    ...
    if not df.empty:
        ...
        plt.plot(df["time"], df["density"], marker="o", label=f"{mode} density")
        plt.title(f"Analytics - {mode}")
        plt.savefig(f"chart_{mode}.png")
    print(f"\n--- SUCCESS: {output_path} ---")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mode", type=str, default="fast", choices=["fast", "stable", "motion", "mask"]
    )
    ...
    if os.path.exists(input_p):
        process_video(input_p, args.output, mode=args.mode)
