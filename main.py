...
def process_video(source_path, output_path, model_id="annonate_datatftphcm/1"):
    ...
    csv_writer = csv.DictWriter(csv_file, fieldnames=["time_sec", "density_pce", "traffic_level", "total_vehicles", "frequency_per_sec"])
    ...
    def callback(frame: np.ndarray, index: int) -> np.ndarray:
        ...
        if index % stride == 0:
            ...
            detections = sv.Detections.from_inference({"predictions": preds, "image": {"width": TARGET_WIDTH, "height": target_height}})
        ...
        if index % fps == 0:
            ...
            csv_file.flush()
            analytics_data.append({"time_sec": index // fps, "density": occ_score, "level": level})

        annotated_frame = frame.copy()
        annotated_frame = box_annotator.annotate(scene=frame_resized, detections=detections)
        labels = [f"#{tid} {cls}" for tid, cls in zip(detections.tracker_id, detections.data['class_name'])] if detections.tracker_id is not None and 'class_name' in detections.data else []
        annotated_frame = label_annotator.annotate(scene=annotated_frame, detections=detections, labels=labels)
        annotated_frame = zone_annotator.annotate(scene=annotated_frame)
        
        # Display Info
        color = (0, 255, 0) if level == "Thong thoang" else (0, 255, 255) if level == "Trung binh" else (0, 0, 255)
        cv2.putText(annotated_frame, f"Density: {occ_score:.1f}% ({level})", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
        cv2.putText(annotated_frame, f"Total Vehicles: {len(unique_ids_seen)}", (10, 45), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
    ...
    with sv.VideoSink(target_path=output_path, video_info=new_video_info, codec="MJPG") as sink:
        for index, frame in enumerate(tqdm(generator, total=video_info.total_frames, desc="Advanced Analytics")):
            sink.write_frame(frame=callback(frame, index))
            if index % 50 == 0: gc.collect() 
    ...
    try:
        ...
        if not df.empty:
            ...
            plt.plot(df['time_sec'], df['density'], color='black', linewidth=2, marker='o', label='Mat do xe (%)')
            plt.title("Bieu do Phan tich Mat do & Muc do Ket")
            plt.xlabel("Thoi gian (Giay)"); plt.ylabel("Mat do (%)")
    ...
    except Exception as e: print(f"Loi ve bieu do: {e}")
...
if __name__ == "__main__":
    ...
    if os.path.exists(input_p):
        process_video(input_p, args.output if args.output.endswith('.avi') else args.output + '.avi')
