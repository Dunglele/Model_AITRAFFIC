...
def process_video(source_path, output_path, model_id="annonate_datatftphcm/1"):
    ...
    new_video_info = sv.VideoInfo(width=TARGET_WIDTH, height=target_height, fps=video_fps, total_frames=video_info.total_frames)
    ...
    def callback(frame: np.ndarray, index: int) -> np.ndarray:
        ...
        if index % stride == 0:
            ...
            if preds:
                img_t = torch.from_numpy(frame_resized).permute(2, 0, 1).float().div(255)
                with torch.inference_mode():
                    outputs = mask_model([img_t])[0]
                
                m_idx = np.where(outputs['scores'].cpu().numpy() > 0.4)[0] # Giảm ngưỡng conf để bắt xe tốt hơn
                if len(m_idx) > 0:
                    masks = (outputs['masks'][m_idx] > 0.5).cpu().numpy().squeeze(1)
                    boxes = outputs['boxes'][m_idx].cpu().numpy()
                    detections = sv.Detections(xyxy=boxes, class_id=outputs['labels'][m_idx].cpu().numpy(), confidence=outputs['scores'][m_idx].cpu().numpy(), mask=masks)
                    detections = detections[np.isin(detections.class_id, [3, 4, 6, 8])]
                    last_detections = tracker.update_with_detections(detections=detections)
                else: last_detections = sv.Detections.empty()
            else: last_detections = sv.Detections.empty()

        mask_area = 0
        if not last_detections.is_empty():
            if last_detections.mask is not None: mask_area = np.sum(last_detections.mask)
            if last_detections.tracker_id is not None:
                for tid in last_detections.tracker_id: unique_ids.add(tid)
        ...
        annotated_frame = mask_annotator.annotate(scene=frame_resized.copy(), detections=last_detections)
        annotated_frame = zone_annotator.annotate(scene=annotated_frame)
        cv2.putText(annotated_frame, f"Density: {occ:.1f}% | Total: {len(unique_ids)}", (10, 15), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 0), 1)
    ...
    with sv.VideoSink(target_path=output_path, video_info=new_video_info, codec="MJPG") as sink:
        for index, frame in enumerate(tqdm(generator, total=video_info.total_frames, desc="Smooth Mask Processing")):
    ...
    if not df.empty:
        ...
        plt.plot(df['time'], df['density'], color='blue', linewidth=2, marker='o', label='Mat do thuc te (Pixel %)')
...
if __name__ == "__main__":
    ...
    if os.path.exists(input_p): process_video(input_p, args.output if args.output.endswith('.avi') else args.output + '.avi')
