...
def process_video(source_path, output_path, model_id="annonate_datatftphcm/1"):
    ...
    csv_writer = csv.DictWriter(csv_file, fieldnames=["time_sec", "density_pce", "traffic_level", "total_vehicles"])
    ...
    def callback(frame: np.ndarray, index: int) -> np.ndarray:
        ...
        if index % stride == 0:
            ...
            for t in tracks:
                ...
                tx.append(t.to_ltrb()); ti.append(t.track_id); tc.append(t.get_det_class())
                
            last_detections = sv.Detections(
                xyxy=np.array(tx), tracker_id=np.array(ti), 
                class_id=np.array([0]*len(ti)), 
                data={'class_name': np.array(tc)}
            ) if tx else sv.Detections.empty()
        ...
        if index % fps == 0:
            csv_writer.writerow({"time_sec": index // fps, "density_pce": f"{occ_score:.1f}", "traffic_level": level, "total_vehicles": len(unique_ids_seen)})
            csv_file.flush()
            analytics_data.append({"time": index // fps, "density": occ_score})

        # Visualize
        annotated_frame = box_annotator.annotate(scene=frame_resized.copy(), detections=detections)
        labels = [f"ID:{tid} {cls}" for tid, cls in zip(detections.tracker_id, detections.data['class_name'])] if not detections.is_empty() and 'class_name' in detections.data else []
        annotated_frame = label_annotator.annotate(scene=annotated_frame, detections=detections, labels=labels)
        annotated_frame = zone_annotator.annotate(scene=annotated_frame)
        
        cv2.putText(annotated_frame, f"StrongSORT Density: {occ_score:.1f}% ({level})", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 255, 255), 2)
    ...
    with sv.VideoSink(target_path=output_path, video_info=new_video_info, codec="MJPG") as sink:
        for index, frame in enumerate(tqdm(generator, total=video_info.total_frames, desc="StrongSORT Analysis")):
    ...
    if not df.empty:
        ...
        plt.plot(df['time'], df['density'], color='orange', linewidth=2, marker='s', label='StrongSORT Density (%)')
        plt.title("Advanced Traffic Analysis (StrongSORT + Appearance Features)")
        plt.xlabel("Time (s)"); plt.ylabel("Density (%)"); plt.legend(); plt.grid(True, linestyle='--')
...
if __name__ == "__main__":
    ...
    if os.path.exists(input_p): process_video(input_p, args.output)
